...
import { setFontSize, setLanguage, clearUser } from '@/store/slices/userSlice';
import useJobBasedPermissions from '@/hooks/useJobBasedPermissions';
import { Tostget } from '@/components/ui/Toast';
import Image from 'next/image';
import { AdminGuard, EmployeeOnly, NonEmployeeOnly, PermissionBasedVisibility } from '@/components/auth/PermissionGuard';
...
function SettingItem({ title, icon, onPress, color = colors.BLACK, width = '48%', display = 'flex' }: SettingItemProps) {
  return (
    <button
      onClick={onPress}
...
export default function SettingsPage() {
  ...
  const { isAdmin, isEmployee } = jobBasedPermissions;
  ...
  return (
    <>
      ...
      <div className="bg-home overflow-hidden w-full">
        ...
        <div className="px-4">
          <div className="flex flex-row flex-wrap justify-between items-start gap-2">
            ...
            {/* Row 1: Language and Pending Operations */}
            <SettingItem
              title="اللغة"
            ...
            {/* Row 2: Employee-only features - Approvals (like mobile app) */}
            <EmployeeOnly>
              <SettingItem
                title="اعتمادات"
