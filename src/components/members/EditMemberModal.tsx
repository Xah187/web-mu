...
export default function EditMemberModal({ user: editUser, onClose, onSuccess }: EditMemberModalProps) {
  ...
  useEffect(() => {
    if (editUser) {
      console.log('Loading user data for edit:', editUser);
      setFormData({
        userName: String(editUser.userName || ''),
        IDNumber: String(editUser.IDNumber || ''),
        PhoneNumber: String(editUser.PhoneNumber || ''),
        job: String(editUser.job || ''),
        jobdiscrption: String(editUser.jobdiscrption || ''),
      });
    }
  }, [editUser]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Original user data:', editUser);

      // إرسال البيانات مع التأكد من الـ headers الصحيحة
      const response = await axiosInstance.put('user/updat', dataToSend, {
        headers: {
